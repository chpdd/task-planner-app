...
from sqlalchemy import ARRAY, Boolean, CheckConstraint, ForeignKey, String, UniqueConstraint
...
class Task(Base):
    ...
    deadline: Mapped[dt.date | None] = mapped_column(nullable=True, default=None)
    ...
    task_executions: Mapped[list["TaskExecution"]] = relationship(
        "TaskExecution", back_populates="task", cascade="all, delete-orphan", passive_deletes=True
    )
