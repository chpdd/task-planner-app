...
from src.schemas.task import UpdateTaskSchema
...
async def create_task(
    request: Request,
    task_schema: schemas.CreateTaskSchema,
    session: db_dep,
    redis: redis_dep,
) -> schemas.TaskSchema:
    ...
    # Изменение задач может повлиять на расписание
...
async def update_task(
    ...
    user_id = request.state.user_id
    await task_crud.schema_owner_get(session, task_id, user_id)
...
async def delete_task(
    ...
    user_id = request.state.user_id
    obj = await task_crud.schema_owner_get(session, task_id, user_id)
    await task_crud.delete(session, obj)
    await session.commit()
